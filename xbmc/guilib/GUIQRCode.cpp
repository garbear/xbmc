...
CGUIQRCode::CGUIQRCode(int parentID, int controlID, float posX, float posY, float width, float height, const CTextureInfo& texture)
...
void CGUIQRCode::UpdateContent(const std::string& content)
{
  ...
  if (UTILS::CQRUtils::EncodeString(content, pixels, width))
  {
    ...
    if (m_qrTexture)
    {
      ...
      m_qrTexture->LoadFromMemory(width, height, width * 4, XB_FMT_A8R8G8B8, true, argbPixels.data());
