...
{
  {
    ...
    {
      const GAME::ControllerPtr controller =
          CServiceBroker::GetGameControllerManager().GetDefaultKeyboard();
      if (controller)
      {
        std::shared_ptr<CFileItem> item =
            std::make_shared<CFileItem>(g_localizeStrings.Get(35150)); // "Keyboard"
  ...
  {
    PeripheralPtr mousePeripheral =
        m_manager.GetByPath(MakeLocation(PeripheralType::PERIPHERAL_MOUSE));
    if (mousePeripheral && mousePeripheral->LastActive().IsValid())
    {
      const GAME::ControllerPtr controller =
          CServiceBroker::GetGameControllerManager().GetDefaultMouse();
      if (controller)
      {
        std::shared_ptr<CFileItem> item =
            std::make_shared<CFileItem>(g_localizeStrings.Get(35171)); // "Mouse"
