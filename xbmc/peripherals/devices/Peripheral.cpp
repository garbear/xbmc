...
void CPeripheral::RegisterInputHandler(IInputHandler* handler, bool bPromiscuous)
{
  ...
  if (it == m_inputHandlers.end())
  {
    ...
    if (addon)
    {
      std::unique_ptr<CAddonInputHandling> addonInput = std::make_unique<CAddonInputHandling>(this, std::move(addon), handler, GetDriverReceiver());
      if (addonInput->Load())
      {
                                          ...
                                          bool bPromiscuous)
{
  ...
  {
    std::unique_ptr<KODI::KEYBOARD::IKeyboardDriverHandler> keyboardDiverHandler;

    PeripheralAddonPtr addon = m_manager.GetAddonWithButtonMap(this);
    if (addon)
    {
      std::unique_ptr<CAddonInputHandling> addonInput = std::make_unique<CAddonInputHandling>(this, std::move(addon), handler);
      if (addonInput->Load())
        keyboardDiverHandler = std::move(addonInput);
    }
    else
    {
      CLog::Log(LOGDEBUG, "Failed to locate add-on for \"{}\"", m_strLocation);
    ...
    {
      std::unique_ptr<KODI::KEYBOARD::CDefaultKeyboardHandling> defaultInput = std::make_unique<KODI::KEYBOARD::CDefaultKeyboardHandling>(this, handler);
      if (defaultInput->Load())
        keyboardDriverHandler = std::move(defaultInput);
    }

    if (keyboardDiverHandler)
    {
      RegisterKeyboardDriverHandler(keyboardDiverHandler.get(), bPromiscuous);
      m_keyboardHandlers[handler] = std::move(keyboardDiverHandler);
    }
  }
}
...
void CPeripheral::RegisterMouseHandler(MOUSE::IMouseInputHandler* handler, bool bPromiscuous)
{
  ...
  if (it == m_mouseHandlers.end())
  {
    ...
    {
      std::unique_ptr<KODI::MOUSE::CDefaultMouseHandling> defaultInput = std::make_unique<KODI::MOUSE::CDefaultMouseHandling>(this, handler);
      if (defaultInput->Load())
