...
bool CAddonInputHandling::Load()
{
  ...
  if (m_buttonMap && m_buttonMap->Load())
  {
    if (m_joystickInputHandler != nullptr)
    {
      m_joystickDriverHandler = std::make_unique<CInputHandling>(m_joystickInputHandler, m_buttonMap.get());
      if (m_joystickDriverReceiver != nullptr)
      {
        m_joystickInputReceiver = std::make_unique<CDriverReceiving>(m_joystickDriverReceiver, m_buttonMap.get());
    ...
    {
      m_keyboardDriverHandler = std::make_unique<KEYBOARD::CKeyboardInputHandling>(m_keyboardInputHandler, m_buttonMap.get());
      return true;
    }
    else if (m_mouseInputHandler != nullptr)
    {
      m_mouseDriverHandler = std::make_unique<MOUSE::CMouseInputHandling>(m_mouseInputHandler, m_buttonMap.get());
      return true;
    }
  }
