                              ...
                              const std::string& itemLabel)
{
  ...
  else
  {
    // Create the list item
    CFileItemPtr item = std::make_shared<CFileItem>(itemLabel);
    item->SetLabel2(CServiceBroker::GetResourcesComponent().GetLocalizeStrings().Get(13298)); // "Disconnected"
    item->SetPath(avatar.GetAddress());
  ...
  if (m_gameClient)
  {
    ...
    else
    {
      ...
      MESSAGING::HELPERS::ShowOKDialogText(
          CVariant{35114},
          CVariant{StringUtils::Format(CServiceBroker::GetResourcesComponent().GetLocalizeStrings().Get(35213), m_gameClient->Name())});
    }
