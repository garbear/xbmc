...
bool CGameClient::Serialize(uint8_t* data, size_t size)
{
  ...
  if (m_bIsPlaying)
  {
    ...
    try
    {
      bSuccess = LogError(m_ifc.game->toAddon->Serialize(m_ifc.game, buffer.data, buffer.size), "Serialize()");
    }
    catch (...)
    {
...
bool CGameClient::Deserialize(const uint8_t* data, size_t size)
{
  ...
  if (m_bIsPlaying)
  {
    ...
    {
      bSuccess =
          LogError(m_ifc.game->toAddon->Deserialize(m_ifc.game, packet.data, packet.size), "Deserialize()");
    }
    catch (...)
    {
