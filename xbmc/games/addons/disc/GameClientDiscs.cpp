...
void CGameClientDiscs::Initialize(const std::string& gamePath, bool usePersistedState)
{
  ...
  if (usePersistedState && persistedStateExists && m_discXml->Load(gamePath, restoredModel))
  {
    ...
    for (size_t i = 0; i < restoredModel.Size(); ++i)
    {
      if (restoredModel.IsRemovedSlotByIndex(i))
        continue;

      const std::string path = restoredModel.GetPathByIndex(i);
      const bool supported = std::any_of(supportedExtensions.begin(), supportedExtensions.end(),
                                         [&path](const std::string& ext)
                                         { return URIUtils::HasExtension(path, ext); });
      if (path.empty() || !supported || !CFileUtils::Exists(path))
      {
        CLog::Log(LOGWARNING,
