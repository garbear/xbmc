...
bool CGameClientCheats::CanInstallCheats()
{
  ADDON::CAddonMgr& addons = CServiceBroker::GetAddonMgr();

  if (addons.IsAddonInstalled(CHEATS_ADDON))
    return addons.IsAddonDisabled(CHEATS_ADDON);

  // By id rather than by listing the repositories: the OSD asks this every
  // time the cheats button's visibility is evaluated
  ADDON::AddonPtr addon;
  return addons.FindInstallableById(CHEATS_ADDON, addon);
}
