...
void CDialogGameLeaderboardEntries::OnInitWindow()
{
  ...
  m_username = settings->GetString(SETTING_RA_USERNAME);
  m_token = settings->GetString(SETTING_RA_TOKEN);
  ...
  while (!m_stopFetch)
  {
    ...
    for (auto it = entries.begin_array(); it != entries.end_array() && !m_stopFetch; ++it)
    {
      ...
      if (dateTs > 0)
      {
        struct tm tmBuf;
#ifdef TARGET_POSIX
        localtime_r(&dateTs, &tmBuf);
#else
        localtime_s(&tmBuf, &dateTs);
#endif
        char dateBuf[32];
        std::strftime(dateBuf, sizeof(dateBuf), "%b %d, %Y", &tmBuf);
      ...
      if (lb.id == m_leaderboardId)
      {
        ...
        for (const auto& item : m_fetchedEntries)
        {
              ...
              item->GetProperty("NotRanked").asString().empty())
          {
            ...
            {
              CLog::Log(LOGWARNING, "CDialogGameLeaderboardEntries: failed to parse rank: {}",
                        e.what());
            }
