...
void CDialogGameAchievements::PopulateList()
{
  ...
  for (const auto& achievement : state.achievements)
  {
    ...
    if (!achievement.rarity.empty())
    {
      try
      {
        const double r = std::stod(achievement.rarity);
        rarityVal = StringUtils::Format("{:.2f}% unlock rate", r);
        if (r > 50.0)
      ...
      {
        CLog::Log(LOGWARNING, "CDialogGameAchievements: failed to parse rarity: {}", e.what());
      }
    }
