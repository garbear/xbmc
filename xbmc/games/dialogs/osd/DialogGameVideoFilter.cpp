...
void CDialogGameVideoFilter::InitVideoFilters()
{
  ...
  std::shared_ptr<CXBMCTinyXML> xml;
  std::string xmlPath;
  ...
  const std::string binAddonPath = URIUtils::AddFileToFolder("special://xbmcbinaddons/", PRESETS_ADDON_NAME);
  const std::string homeAddonPath = URIUtils::AddFileToFolder("special://home/addons/", PRESETS_ADDON_NAME);

  for (const auto& basePath : { binAddonPath, homeAddonPath })
  {
    xmlPath = URIUtils::AddFileToFolder(basePath, "resources", xmlFilename);
    if (XFILE::CFile::Exists(xmlPath))
    {
      xml = std::make_shared<CXBMCTinyXML>(xmlPath);
      if (xml->LoadFile())
        break;

      CLog::LogF(LOGERROR, "Couldn't load shader presets from XML, {}",
  ...
  while ((child = root->IterateChildren(child)))
  {
    ...
    if ((pathNode = child->FirstChild("path")))
      if ((pathNode = pathNode->FirstChild()))
        videoFilter.path = URIUtils::AddFileToFolder(URIUtils::GetBasePath(xmlPath), pathNode->Value());
