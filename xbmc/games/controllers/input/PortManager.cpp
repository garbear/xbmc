...
void CPortManager::LoadXML()
{
  ...
  DeserializePorts(pRootElement, m_controllerTree.Ports());
}
...
void CPortManager::SaveXML()
{
  ...
  SerializePorts(node, m_controllerTree.Ports());
...
{
  ConnectController(portAddress, connected, controllerId, m_controllerTree.Ports());
}
                                     ...
                                     CPortNode& port)
{
  ...
  // Check children
  return ConnectController(portAddress, connected, controllerId, port.CompatibleControllers());
}
...
{
  for (CPortNode& childPort : controller.Hub().Ports())
  {
...
void CPortManager::DeserializePorts(const TiXmlElement* pElement, PortVec& ports)
{
       ...
       pPort = pPort->NextSiblingElement())
  {
    ...
    auto it = std::find_if(ports.begin(), ports.end(),
                           [&portId](const CPortNode& port) { return port.PortID() == portId; });
    if (it != ports.end())
    {
      CPortNode& port = *it;
...
void CPortManager::DeserializePort(const TiXmlElement* pPort, CPortNode& port)
{
  ...
  port.SetActiveController(activeControllerId);

  DeserializeControllers(pPort, port.CompatibleControllers());
}
       ...
       pController = pController->NextSiblingElement())
  {
    ...
    auto it = std::find_if(controllers.begin(), controllers.end(),
                           [&controllerId](const CControllerNode& controller) {
                             return controller.Controller()->ID() == controllerId;
                           });
...
{
  // Child ports
  DeserializePorts(pController, controller.Hub().Ports());
}
...
{
  // Port ID
  portNode.SetAttribute(XML_ATTR_PORT_ID, port.PortID());

  // Port address
  portNode.SetAttribute(XML_ATTR_PORT_ADDRESS, port.Address());

  // Connected state
  portNode.SetAttribute(XML_ATTR_PORT_CONNECTED, port.Connected() ? "true" : "false");

  // Active controller
  if (port.ActiveController().Controller())
  {
    const std::string controllerId = port.ActiveController().Controller()->ID();
    portNode.SetAttribute(XML_ATTR_PORT_CONTROLLER, controllerId);
  }

  // All compatible controllers
  SerializeControllers(portNode, port.CompatibleControllers());
}
                                        ...
                                        const ControllerNodeVec& controllers)
{
  ...
  {
    // Skip controller if has no state
    if (controller.Hub().Ports().empty())
      continue;
...
{
  // Controller ID
  controllerNode.SetAttribute(XML_ATTR_CONTROLLER_ID, controller.Controller()->ID());

  // Ports
  SerializePorts(controllerNode, controller.Hub().Ports());
}
