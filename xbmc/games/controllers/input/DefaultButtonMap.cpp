...
bool CDefaultButtonMap::Load()
{
  ...
  CLog::Log(LOGDEBUG, "Failed to load button map for \"{}\" with profile {}", m_device->Location(), m_strControllerId);
  return false;
}
...
bool CDefaultButtonMap::GetFeature(const JOYSTICK::CDriverPrimitive& primitive, JOYSTICK::FeatureName& feature)
{
  switch (primitive.Type())
  {
    ...
    {
      std::string featureName = CDefaultMouseTranslator::TranslateMouseButton(primitive.MouseButton());
      if (!featureName.empty())
      {
        feature = std::move(featureName);
        return true;
    ...
    {
      std::string featureName = CDefaultMouseTranslator::TranslateMousePointer(primitive.PointerDirection());
      if (!featureName.empty())
      {
        feature = std::move(featureName);
        return true;
