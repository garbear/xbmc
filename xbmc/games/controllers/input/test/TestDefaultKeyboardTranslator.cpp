...
{
// Helper function
KEYBOARD::KeySymbol GetKeycode(KEYBOARD::KeySymbol keysym, const ControllerPtr& controller, unsigned int& count)
{
  std::string keyName = CDefaultKeyboardTranslator::TranslateKeycode(keysym);
  ++count;
...
TEST(TestDefaultKeyboardTranslator, TranslateKeycode)
{
  ...
  const ControllerPtr controller = CServiceBroker::GetGameServices().GetController(DEFAULT_KEYBOARD_ID);
