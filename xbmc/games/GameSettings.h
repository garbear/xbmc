...
public:
  ...
  AchievementState MarkAchievementEarned(unsigned int achievementId, bool& newlyEarned)
  {
    std::lock_guard<std::mutex> lock(m_achievementMutex);
    newlyEarned = false;
    for (AchievementInfo& info : m_achievementState.achievements)
    {
      if (info.id == achievementId)
      {
        ...
        {
          info.earned = true;
          ++m_achievementState.unlockedAchievements;
          newlyEarned = true;
        }
        break;
      }
