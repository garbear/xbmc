...
void CRPRendererOpenGL::Render(uint8_t alpha)
{
  ...
  const auto key = std::tuple<CRenderBufferOpenGL*, unsigned int, unsigned int>{
      renderBuffer, static_cast<unsigned int>(m_fullDestWidth),
      static_cast<unsigned int>(m_fullDestHeight)};
  const auto it = m_RBTexturesMap.find(key);
  ...
  else
  {
    ...
    m_RBTexturesMap.emplace(key, rbTextures);
  }
