...
/*!
 * \brief Image cache cleanup
 */
static void CleanImageCacheIfNeeded();

/*!
 * \brief Offline award queue helper
 */
...
bool CCheevos::LoadData()
{
  ...
  // TMSG_UPDATE_PLAYER_ITEM takes ownership and adopts this pointer into a unique_ptr
  ...
  for (auto it = achievements.begin_array(); it != achievements.end_array(); ++it)
  {
    ...
    if (flags == FLAGS_CORE)
    {
      ...
      {
        std::lock_guard<std::mutex> lock(m_cheevoTitlesMutex);
        m_cheevoTitles[id] = {title, badgeUrl};
      }
    }
  }
  ...
  for (const auto& [id, fields] : m_activatedCheevoMap)
  {
    ...
    if (fields.size() > 4)
    {
      try
      {
        info.points = static_cast<unsigned int>(std::stoul(fields[4]));
      }
  ...
  if (!m_gameTitle.empty() && !m_activatedCheevoMap.empty())
  {
    ...
    const std::string body =
        unlockCountKnown
            ...
            : StringUtils::Format("{} achievements available", m_activatedCheevoMap.size());
...
static void QueueAward(const std::string& url)
{
  ...
  if (outFile.OpenForWrite(CURL(RA_AWARD_QUEUE_FILE), false))
  {
    ...
    if (outFile.Seek(0, SEEK_END) < 0)
    {
      CLog::Log(LOGERROR, "CCheevos: failed to seek to end of award queue");
      return;
    }
...
void CCheevos::CallbackUrlId(const std::string& achievementUrl, unsigned int cheevoId)
{
  ...
  // Atomically mark the achievement earned and update the shared unlocked count
  {
    ...
    const auto state = CServiceBroker::GetGameServices().GameSettings().MarkAchievementEarned(
        cheevoId, newlyEarned);
...
void CCheevos::CheckTriggeredAchievement()
{
  ...
  m_gameClient->Cheevos().GetAchievementUrlId(
      [this](const std::string& achievementUrl, unsigned int cheevoId)
  {
