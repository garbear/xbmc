...
void CShaderPresetGL::SetVideoSize(unsigned int videoWidth, unsigned int videoHeight)
{
  ...
  for (unsigned int shaderIdx = 0; shaderIdx < numPasses; ++shaderIdx)
  {
    ...
    else
    {
      ...
      m_pShaderTextures.emplace_back(
          new CShaderTextureGL(*textureGL, nextPass.mipmap, pass.fbo.sRgbFramebuffer));
    }
