                                   ...
                                   IShaderTexture* target)
{
  ...
  unsigned int numPasses = static_cast<unsigned int>(m_pShaders.size());
...
bool CShaderPresetDX::CreateShaders()
{
  ...
  for (unsigned int shaderIdx = 0; shaderIdx < numPasses; ++shaderIdx)
  {
    ...
    if (!videoShader->Create(shaderSource, shaderPath, passParameters, std::move(passLUTsDX),
                             m_outputSize, shaderIdx, pass.frameCountMod))
    {
