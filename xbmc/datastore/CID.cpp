...
std::string CCID::ToString() const
{
  ...
  const char base58BtcPrefix =
      static_cast<char>(CRYPTO::CMultibase::TranslateEncoding(CRYPTO::CMultibase::Encoding::BASE58BTC));
...
void CCID::ParseBytes()
{
  ...
  const auto decodeWithConsumedBytes = [](const uint8_t* source, size_t sourceSize,
