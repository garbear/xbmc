...
//--==----==----==----==----==----==----==----==----==----==----==----==----==--
  ...
  ///
  virtual GAME_ERROR GetRichPresenceEvaluation(char* evaluation, size_t size, int consoleID)
  {
    return GAME_ERROR_NOT_IMPLEMENTED;
  }
...
private:
  void SetAddonStruct(KODI_ADDON_INSTANCE_STRUCT* instance)
  {
    ...
    instance->game->toAddon->EnableRichPresence = ADDON_EnableRichPresence;
    instance->game->toAddon->GetRichPresenceEvaluation = ADDON_GetRichPresenceEvaluation;
  ...
  {
    return static_cast<CInstanceGame*>(instance->toAddon->addonInstance)
        ->GetRichPresenceEvaluation(evaluation, size, consoleID);
  }
