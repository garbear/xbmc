...
{
  if (!m_data.empty())
    return TranslateEncoding(m_data[0]);
...
{
  return std::make_pair(GetDataPtr(), GetDataSize());
}
...
void CMultibase::SetData(Encoding encoding, const uint8_t* data, size_t size)
{
  ...
  {
    CLog::Log(LOGERROR, "CMultibase::SetData - invalid null data pointer with size {}", size);
    return;
  }

  std::vector<uint8_t> multibaseData;

  // Reserve the space for encoding byte + payload.
  multibaseData.reserve(size + 1);

  // Add the encoding byte.
  multibaseData.push_back(TranslateEncoding(encoding));

  if (size > 0)
    multibaseData.insert(multibaseData.end(), data, data + size);

  m_data = std::move(multibaseData);
}

void CMultibase::SetData(const std::string& multibase)
{
  m_data.assign(multibase.begin(), multibase.end());
}
...
void CMultibase::SetData(const uint8_t* multibaseData, size_t multibaseSize)
{
  ...
  {
    CLog::Log(LOGERROR, "CMultibase::SetData - invalid null data pointer with size {}",
              multibaseSize);
    return;
  }
  ...
  if (multibaseSize == 0)
    ...
    m_data.assign(multibaseData, multibaseData + multibaseSize);
}
