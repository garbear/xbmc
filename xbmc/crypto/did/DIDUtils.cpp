...
bool CDIDUtils::ParseDID(const std::string& did, std::string& method, std::string& identifier)
{
  ...
  static const std::regex regex("^did:([A-Za-z0-9]+):([^#;/?\\s]+)(?:[;/?#].*)?$");
...
{
  COpenSSLRandomGenerator rng;
  const std::vector<uint8_t> bytes = rng.RandomBytes(32);
  if (bytes.empty())
    return {};

  return CBase58BTC::EncodeBase58(bytes.data(), bytes.size());
}

std::string CDIDUtils::GenerateIpnsName(const PrivateKey& key)
{
  // IPNS peer ID derivation from private keys is not implemented yet. Return an
  // empty identifier so callers fail safely instead of producing did:ipid:.
  return {};
}
