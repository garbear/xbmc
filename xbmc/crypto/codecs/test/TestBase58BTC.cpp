...
TEST(TestBase58BTC, DecodeBase58WhitespaceOnly)
{
  // Base58 decoding follows the Bitcoin behavior of allowing surrounding
  // whitespace, so whitespace-only input decodes as empty data.
  std::vector<uint8_t> decoded = {0x01, 0x02, 0x03};
  bool success = CBase58BTC::DecodeBase58("    ", decoded);
  EXPECT_TRUE(success);
  EXPECT_TRUE(decoded.empty());
}
...
TEST(TestBase58BTC, DecodeWithTrailingSpaces)
{
  std::string encoded = "ZiCa    ";
  std::vector<uint8_t> decoded;
  bool success = CBase58BTC::DecodeBase58(encoded, decoded);
...
TEST(TestBase58BTC, DecodeRejectsTrailingGarbage)
{
  std::vector<uint8_t> decoded = {0x01, 0x02, 0x03};
  bool success = CBase58BTC::DecodeBase58("ZiCa garbage", decoded);
  EXPECT_FALSE(success);
  EXPECT_EQ(decoded, std::vector<uint8_t>({0x01, 0x02, 0x03}));
}

TEST(TestBase58BTC, DecodeInvalidDoesNotOverwriteOutput)
{
  std::vector<uint8_t> decoded = {0x01, 0x02, 0x03};
  bool success = CBase58BTC::DecodeBase58("Zi$#a", decoded);
  EXPECT_FALSE(success);
  EXPECT_EQ(decoded, std::vector<uint8_t>({0x01, 0x02, 0x03}));
}

TEST(TestBase58BTC, ValidDecodeReplacesOutput)
{
  std::vector<uint8_t> decoded = {0x01, 0x02, 0x03};
  bool success = CBase58BTC::DecodeBase58("ZiCa", decoded);
  EXPECT_TRUE(success);
  EXPECT_EQ(decoded, std::vector<uint8_t>({0x61, 0x62, 0x63}));
}

TEST(TestBase58BTC, EncodeNullPointer)
{
  EXPECT_EQ("", CBase58BTC::EncodeBase58(nullptr, 0));

  // The string-returning API cannot distinguish invalid null input from valid
  // empty input, so keep the legacy return value documented in Base58BTC.h.
  EXPECT_EQ("", CBase58BTC::EncodeBase58(nullptr, 1));
}
