...
KeyPair COpenSSLEd25519Provider::Generate()
{
  ...
  if (1 != EVP_PKEY_get_raw_public_key(keyStruct.get(),
      keyPair.publicKey.data.data(), &pubLen))
    return {};

  // Fill buffer with raw private key data
  if (1 != EVP_PKEY_get_raw_private_key(keyStruct.get(),
      keyPair.privateKey.data.data(), &privLen))
    return {};
