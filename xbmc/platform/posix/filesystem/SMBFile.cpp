...
bool CSMBFile::ReopenAtPositionLocked(int64_t offset)
{
  ...
  errno = 0;
  const int newFd = smbc_open(m_reopenPath.c_str(), O_RDONLY, 0);
  if (newFd < 0)
  {
    if (errno == 0)
      errno = EIO;
...
ssize_t CSMBFile::Read(void* lpBuf, size_t uiBufSize)
{
  ...
  if (m_reopenPath.empty() ||
      (!unexpectedEof && !SMBFileRecovery::IsReconnectableReadError(readErrno)))
  {
    if (bytesRead < 0)
  ...
  for (const auto backoff : SMB_RECONNECT_BACKOFF)
  {
    ...
    if (!ReopenAtPositionLocked(failedReadOffset))
    {
