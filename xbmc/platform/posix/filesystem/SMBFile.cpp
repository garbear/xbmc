...
bool CSMBFile::ReopenAtPositionLocked(int64_t offset, const std::string& reopenPath)
{
  ...
  errno = 0;
  const int64_t actualOffset = m_fileOperations.Seek(newFd, offset, SEEK_SET);
...
ssize_t CSMBFile::Read(void* lpBuf, size_t uiBufSize)
{
  ...
  if (m_fd == -1)
  {
    ...
    if (!BeginRecoveryAttemptLocked() || !ReopenAtPositionLocked(reopenOffset, reopenPath))
    {
...
int64_t CSMBFile::Seek(int64_t iFilePosition, int iWhence)
{
  ...
  {
    if (!AddOffset(m_fileSize, iFilePosition, target))
    {
      errno = EOVERFLOW;
      return -1;
    }
  ...
  {
    errno = 0;
    const int64_t pos = m_fileOperations.Seek(m_fd, target, SEEK_SET);
    if (pos == target)
    {
    ...
    const int seekErrno = pos < 0 && errno != 0 ? errno : EIO;
    if (pos >= 0)
    {
      const int failedFd = m_fd;
      m_fd = -1;
    ...
    {
      CLog::Log(LOGERROR, "{} - Error( {}, {}, {} )", __FUNCTION__, pos, seekErrno,
                strerror(seekErrno));
      errno = seekErrno;
      return -1;
    }
  ...
  while (true)
  {
    ...
    if (ReopenAtPositionLocked(target, reopenPath))
    {
      m_readPosition = target;
      m_reopenOnNextRead = false;
      m_reopenInProgressGeneration = 0;
