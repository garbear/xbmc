...
void CRos2InputPublisher::Initialize()
{
  const std::string publishPeripheralsTopic =
      BuildRosName(m_rosNamespace, {m_hostname, PUBLISH_PERIPHERALS_TOPIC});
  const std::string publishInputTopic =
      BuildRosName(m_rosNamespace, {m_hostname, PUBLISH_INPUT_TOPIC});
  const std::string serviceCaptureInput =
      BuildRosName(m_rosNamespace, {m_hostname, SERVICE_CAPTURE_INPUT});
